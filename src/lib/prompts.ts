...
const FIX_HINT_GUIDE: Record<string, string> = {
  ...
  "정확도/근거 기반 표현":
    "검색 근거에 없는 수치/날짜/출시 정보는 단정하지 말고, 확인 필요 문구로 완화하세요.",
  "AI 티 문체 점검":
...
- 검색 근거에 없는 가격/날짜/통계/출시 정보를 사실처럼 단정하지 마세요.
...
## 정확도 가이드
- 가격/날짜/수치/출시시점/통계처럼 검증이 필요한 정보는 최신 참고 정보에 있는 범위에서만 작성하세요.
- 근거가 불충분하면 "공식 페이지에서 최신 확인이 필요합니다"처럼 완화 표현으로 바꾸세요.
- 확인되지 않은 고유명사, 기관명, 수상 이력, 순위/점유율 수치를 임의로 만들지 마세요.
...
- 검색 근거에 없는 가격/날짜/통계/출시정보는 단정 문장으로 쓰지 마세요.
- 근거가 약한 사실 문장은 "확인 필요/경우에 따라 다름"으로 완화하세요.
